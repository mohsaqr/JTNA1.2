...
TNAClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    ...
    private = list(
        .run = function() {
            ...
            if(self$results$buildModelContent$isFilled()) {
                ...
                tryCatch({
                    ...
                    if(!is.null(dataForTNA)) {
                        ...
                        if(type == "attention") {
                            # Attention weights every event by its full decayed
                            # history, so the build can be slow on long sequences.
                            # Show a heads-up, then build.
            ...
            if(!is.null(model) && self$options$cliques_show_plot) {
        ...
        .showCommunityPlot=function(image, ...) {
            ...
            if(!is.null(plotData) && self$options$community_show_plot)  {
                tryCatch({
                    methods <- self$options$community_methods
                    plot(x=plotData, method=methods, layout=self$options$community_plot_layout, background="transparent")
                }, error = function(e) {
